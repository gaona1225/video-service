...
let findMobileUserByName = (name) => {
    let _sql = `select * from mobileusers where userName="${name}";`
    return query(_sql);
}
...
let getCommentById = (id) => {
    var _sql = `select * from comments where id="${id}";`
    return query(_sql);
}
// 通过用户名获取评论
...
// 增加like
let addLike = (value) => {
    var _sql = `insert into likes set isLike=?,userName=?,videoName=?,videoImg=?,star=?,uid=?;`
    return query(_sql, value);
}
// 获取单个video里的用户like状态
...
let getLikeList = (name, num) => {
    var _sql = `select * from likes where userName="${name}" AND isLike="${num}";`
    return query(_sql);
}
// 获取喜欢的数量
let getLikeStar = (type, uid) => {
    var _sql = `select count(*) from likes where isLike="${type}" AND uid="${uid}";`
    return query(_sql);
}
